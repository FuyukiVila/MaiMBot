...
class SubHeartflow:
    ...
    async def do_a_thinking(self):
        ...
        reponse, reasoning_content = await self.llm_model.generate_response_async(
            prompt
        )
    ...
    async def judge_willing(self):
        ...
        response, reasoning_content = await self.llm_model.generate_response_async(
            prompt
        )
        # 解析willing值
        ...
        logger.info(
            f"{self.observe_chat_id}麦麦的回复意愿：{self.current_state.willing}"
        )
