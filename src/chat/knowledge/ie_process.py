...
def _entity_extract(llm_req: LLMRequest, paragraph: str) -> List[str]:
    ...
    try:
        ...
        response, (reasoning_content, model_name) = asyncio.run(llm_req.generate_response_async(entity_extract_context))
...
def _rdf_triple_extract(llm_req: LLMRequest, paragraph: str, entities: list) -> List[List[str]]:
    ...
    try:
        ...
        # 如果没有运行中的事件循环，直接使用 asyncio.run
        response, (reasoning_content, model_name) = asyncio.run(llm_req.generate_response_async(rdf_extract_context))
