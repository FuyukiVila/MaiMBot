...
class WillingManager:
        ...
        while True:
            ...
            for chat_id in self.chat_reply_willing:
                self.chat_reply_willing[chat_id] = max(
                    0, self.chat_reply_willing[chat_id] * 0.6
                )
            for chat_id in self.chat_reply_willing:
                self.chat_reply_willing[chat_id] = max(
                    0, self.chat_reply_willing[chat_id] * 0.6
    ...
    async def change_reply_willing_received(
        ...
        # 保证意愿值不少于0.6
        if not is_mentioned_bot and current_willing < 0.6:
            current_willing = 0.6
            print(f"未被提及, 当前意愿: {current_willing}")
    ...
    def change_reply_willing_after_sent(self, chat_stream: ChatStream):
        ...
        if stream:
            ...
            if current_willing < 1:
                self.chat_reply_willing[stream.stream_id] = min(
                    1, current_willing + 0.2
                )
