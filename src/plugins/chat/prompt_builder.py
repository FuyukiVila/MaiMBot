...
class PromptBuilder:
    ...
    async def _build_prompt(
        ...
        bot_schedule_now_time, bot_schedule_now_activity = (
        ...
        if stream_id:
            chat_talking_prompt = get_recent_group_detailed_plain_text(
                self.db, stream_id, limit=global_config.MAX_CONTEXT_SIZE, combine=True
            )
        ...
        # 额外信息要求
        extra_info = """但是记得回复简短一些，尤其注意在没明确提到时不要过多提及自身的背景, 不要直接回复别人发的表情包，记住不要输出多余内容(包括前后缀，冒号和引号，括号，表情等)，只需要输出回复内容就好，不要输出其他任何内容"""
    ...
    def _build_initiative_prompt_select(
        self, group_id, probability_1=0.8, probability_2=0.1
    ):
        current_date = time.strftime("%Y-%m-%d", time.localtime())
        current_time = time.strftime("%H:%M:%S", time.localtime())
        bot_schedule_now_time, bot_schedule_now_activity = (
    ...
    async def get_prompt_info(self, message: str, threshold: float):
        related_info = ""
        logger.debug(
            f"获取知识库内容，元消息：{message[:30]}...，消息长度: {len(message)}"
        )
